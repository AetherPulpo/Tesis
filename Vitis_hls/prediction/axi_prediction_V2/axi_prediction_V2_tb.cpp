...
int main(){
    ...
    float test_max;
    float tmp_tremor;
    axi_stream expected_result[DATA_SIZE];
    axi_stream result;
    ...
    hls::stream<axi_stream> test_ctremor_output_stream ("stream_salida_current_tremor");
    hls::stream<axi_stream> test_ptremor_output_stream ("stream_salida_pred_tremor");

    axi_stream test_result[DATA_SIZE];
    axi_stream test_result_ptremor[DATA_SIZE];
    axi_stream result_ptremor;
    axi_stream expected_result_ptremor [DATA_SIZE];
	FILE *fpr1;
	FILE *fpr2;
	FILE *fpr3;
	FILE *fpr4;
    ...
    //test_output_stream va despues de max_input y antes de test_ctremor
    //Se aplica la funcion axi tremor_amp
    axi_prediction_V2(test_input_stream, test_amp_input_stream ,test_max_input_stream, test_ctremor_output_stream, test_ptremor_output_stream);
    ...
    for(int i = 0; i < DATA_SIZE; i++){
    	//tmp_tremor = test_output_stream.read();
    	tmp_tremor = input_read[i];
    	std::cout << "Entrada["<< i <<"] : " << input_read[i] << " Salida : "<< tmp_tremor <<  std::endl;
    	assert(abs(input_read[i] - tmp_tremor) <= 0.001);
    }
    std::cout << "burst_index : " << test_result[0].data << " Valor esperado: "<< expected_result[0].data <<  std::endl;
    std::cout << "burst_index TLAST? : " << test_result[0].last <<  std::endl;
    std::cout << "time for next burst : " << test_result_ptremor[0].data << " Valor esperado: "<< expected_result_ptremor[0].data <<  std::endl;
    std::cout << "time for next burst  TLAST? : " << test_result_ptremor[0].last  <<  std::endl;

    //Se compara el resultado con el resultado esperado
    assert(abs(test_result[0].data - expected_result[0].data) <= 1);
    assert(abs(test_result_ptremor[0].data - expected_result_ptremor[0].data) <= 1);
    ...
    for (int i = 1; i < 5; i++){
		std::cout << "burst_index : " << test_result[i].data << " Valor esperado: "<< expected_result[i].data <<  std::endl;
		std::cout << "burst_index TLAST? : " << test_result[i].last <<  std::endl;
		//Se compara el resultado con el resultado esperado
		assert(abs(test_result[i].data - expected_result[i].data) <= 0.001);
		if (i < 4){
			std::cout << "time for next burst : " << test_result_ptremor[i].data << " Valor esperado: "<< expected_result_ptremor[i].data <<  std::endl;
			std::cout << "time for next burst  TLAST? : " << test_result_ptremor[i].last  <<  std::endl;
			//Se compara el resultado con el resultado esperado

			assert(abs(test_result_ptremor[i].data - expected_result_ptremor[i].data) <= 0.001);
		}
    }
