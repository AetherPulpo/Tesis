...
int main(){
    ...
    axi_cut_N(test_input_stream, test_output_stream);
    ...
    for (int i = 0; i < FILTER_DATA_SIZE; i++){
    	if ( i > CUT_SIZE-1){
    		...
    		std::cout << "Salida [" << (i-204) <<"]: " << test_result[(i-204)] << " Valor esperado: "<< expected_result[i] <<  std::endl;
    		//Se compara el resultado con el resultado esperado
    		assert(test_result[i-204] == expected_result[i]);
    	}
    	else{
    		std::cout << "Entrada ["<< i <<"]: " << input_read[i] <<std::endl;
    	}
